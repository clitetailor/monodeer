...
export class CaptureResult {
  constructor(private doc: string, private indexes: [string, number][] = []) {}
...
export class CursorTest {
  ...
  capture(input: string, testOptions?: CursorTestOptions): CaptureResult {
  ...
  inline(input: string, testOptions?: CursorTestOptions): CaptureResult {
  ...
  private _inline(
    ...
    while (!cursor.isEof()) {
      if (cursor.startsWith(prefix)) {
        ...
        chunks.push(marker.readTo(cursor));
  ...
  block(input: string, _testOptions: CursorTestOptions = {}): CaptureResult {
    const cursor = Cursor.from(input);
    const chunks: string[] = [];
    ...
    while (!cursor.isEof()) {
      const regexpArray = cursor.exec(/^[ \t]*([0-9]+)?[ \t]*\|[ \t](.*?)$/gmy);
